...
{
    public partial class ElectronicReportingControl : UserControl
    {
        ...
        public ElectronicReportingControl(Session session)
        {
            InitializeComponent();
            _session = session;
        }
        ...
        private async void Form_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (statusCustomer != null)
                {
                    critreia = new BinaryOperator($"{nameof(Customer.CustomerStatus)}.{nameof(CustomerStatus.Status)}", statusCustomer);
                }
            }
                    ...
                    try
                    {
                        ...
                        if (electronicReporting != null)
                        {
                            ...
                            foreach (var obj in electronicReporting.ElectronicReportingСustomerObjects.Where(w => w.DateTo >= dateSince && w.DateTo <= dateTo))
                            {
                                try
                                {
                                    var objectControl = new ElectronicReportingObjectControl(customer, obj, dateTo) { Dock = DockStyle.Top };
                                    ...
                                    if (objectControl.IsCongratulate)
                                    {
                                        ...
                                        objectControl.ElectronicReportingСustomerObjectEditEvent += ObjectControl_ElectronicReportingСustomerObjectEditEvent;
                                    }
                                }
                                catch (Exception) { }
                            }
        ...
        private void ObjectControl_ElectronicReportingСustomerObjectEditEvent(object sender)
        {
            var objectControl = sender as ElectronicReportingObjectControl;
